import math,app.constants,app.config
...
def browserOptions():
    options = Options()
    # firefoxProfileRootDir = app.config.firefoxProfileRootDir
...
def jobsToPages(numOfJobs: str) -> int:
  ...
  if (' ' in numOfJobs):
    ...
    number_of_pages = math.ceil(totalJobs_int/app.constants.jobsPerPage)
...
class LinkedinUrlGenerate:
    def generateUrlLinks(self):
        path = []
        for location in app.config.location:
            for keyword in app.config.keywords:
                    url = app.constants.linkJobUrl + "?f_AL=true&keywords=" +keyword+self.jobType()+self.remote()+self.checkJobLocation(location)+self.jobExp()+self.datePosted()+self.salary()+self.sortBy()
                    path.append(url)
        return path
    ...
    def jobExp(self):
        jobtExpArray = app.config.experienceLevels
    ...
    def jobType(self):
        jobTypeArray = app.config.jobType
